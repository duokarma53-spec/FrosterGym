...
export function Staff() {
  ...
  async function loadStaff() {
    ...
    try {
      ...
      const { data: profilesData } = await supabase
        .from('profiles')
        .select('email, is_active, id, user_id')
        .eq('gym_id', gym!.id);
      ...
      const mappedStaff = (staffData || []).map((s: any) => {
        ...
        return {
          ...
          profile_id: prof ? prof.user_id : null // Use user_id for permissions!
        };
      });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // the profile created by the database trigger might not be immediately available.
      let newProfile = null;
      let retries = 0;

      while (!newProfile && retries < 10) {
        // Attempt to update the profile. This might update 0 rows initially if not visible yet.
        await tempClient
          .from('profiles')
          .update({
            role: 'staff',
        ...
        // Try to fetch it using Owner's client to confirm it was updated
      ...
      // 4. Clean up the dummy gym generated by DB trigger
